#ifndef _CORENEURONT__LOGGING_HPP_
#define _CORENEURONT__LOGGING_HPP_

#include <iomanip>
#ifndef NDEBUG
  #include <iostream>  // used by CORENEURON::log::test_logging_levels()
#endif
#include <string>
using namespace std;
...
/**
 * General variables for coloured terminal output.
 *
 */
struct OUT
{
...
#ifdef CORENEURONT_NO_LOGGING
  #define ELPP_DISABLE_LOGS
  #define ML_NOLOG
#endif

#include <CORENEURON/easylogging++.h>
...
//! @}
...
#ifndef CORENEURONT_LOGGER_INITIALIZED
  // initialize easyloggingpp
  // FIXME: this might already be called by code using CORENEURONT++
   ...
   *
   * @note In case the executable using CORENEURONT++ is also using easylogging++ as its logging library
   *   and initializing it prior to including the CORENEURONT++ headers, please define this symbol prior
   *   including any of the CORENEURONT++ headers.
   */
  #define CORENEURONT_LOGGER_INITIALIZED
#endif

#ifndef CORENEURONT_LOGGER_DEFAULT_GLOBAL_MILLISECOND_WIDTH
  //! precision of milliseconds to be printed
  #define CORENEURONT_LOGGER_DEFAULT_GLOBAL_MILLISECOND_WIDTH "2"
#endif
  ...
  #define VLOG_FUNC_START(scope) \
    CORENEURON::log::stack_position++; \
    VLOG(9) << std::string((CORENEURON::log::stack_position - 1) * 2, ' ') << "START: " << std::string(scope) << "::" << std::string(__func__) << "()"
#endif
  ...
  #define VLOG_FUNC_END(scope) \
    CORENEURON::log::stack_position--; \
    VLOG(9) << std::string(CORENEURON::log::stack_position * 2, ' ') << "DONE:  " << std::string(scope) << "::" << std::string(__func__) << "()";
#endif
 ...
 *
 * This uses CORENEURON::log::stack_position to create a spacing string twice as long.
 */
#define LOG_INDENT \
  string(CORENEURON::log::stack_position * 2, ' ')
 ...
 *
 * @see CORENEURON::log::add_custom_logger()
 */
...
namespace CORENEURON
{
   ...
   *
   * As CORENEURONT++ is using easylogging++ as the logging backend, there are six distinced logging
   * levels plus ten additional verbose logging levels.
   *
   * To achieve consistent logging throughout CORENEURONT++ and its examples, we agree on the following
  ...
  namespace log
  {
     ...
     *
     * The local rank number as given by CORENEURON::config::get_rank() is filled from the left with the
     * @p fill character.
     *
    ...
    inline string format_mpi_rank(const char fill = ' ')
    {
      ...
      frmter << std::setw(4) << std::setfill(fill) << CORENEURON::config::get_rank();
      return frmter.str();
    }
                                           ...
                                           const el::Configurations* default_conf = nullptr)
    {
      ...
      if (default_conf) {
        ...
        milliseconds_width = CORENEURONT_LOGGER_DEFAULT_GLOBAL_MILLISECOND_WIDTH;
        to_stdout = CORENEURON::config::get_value<std::string>("stdout", "true");
      }
    ...
    {
      bool colorize = CORENEURON::config::get_value<bool>("color", true);
    ...
    inline static void load_default_config()
    {
      ...
      CORENEURON::log::initialized = true;
     ...
     *
     * @note This function should not be called directly from user code but through CORENEURON::init.
     *
     */
    inline static void start_log(int argc, char** argv)
    {
      ...
      set_logging_flags();
      load_default_config();
      CORENEURON::log::stack_position = 0;
      ML_CLOG(INFO, "default", "CORENEURONT++ version " << CORENEURON::VERSION);
    }
  }  // ::CORENEURON::log
}  // ::CORENEURON

#endif  // _CORENEURONT__LOGGING_HPP_
